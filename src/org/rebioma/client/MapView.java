...
public class MapView extends ComponentView implements CheckedSelectionListener,
		...
		TileLayerCallback, ItemSelectionListener, SelectionHandler<Integer>,
		OccurrencePageSizeChangeHandler, GeocoderRequestHandler,
		MapDrawingControlListener {
	...
	protected void mapGeocoderResult(
		...
		for (int i = 0; i < results.length(); i++) {
			...
			Marker geocoderMarker = GeocoderControl
	...
	private void showWindowInfo(OccurrenceMarkerManager occurrenceMarkerManager) {
		...
		infoWindowOptions.setPosition(occurrenceMarkerManager.getMarker()
				.getPosition());
		try {
			infoWindowOptions.setContent(summaryContent);
		} catch (JavaScriptException e) {
			// on reessaie
			// TODO determiner pourquoi pour un nombre paire(2ième, 4ième, ...)
			// d'execution, il y a une JavaScriptException
			infoWindowOptions.setContent(summaryContent);
		}
