...
public class OccurrenceQuery extends Query<Occurrence> {
	 ...
	 * contain concept and value pairs that match specific occurrence search
	 * type (i.e All Occurrence: this filter is empty).
	 */
	...
	/**
	 * filtre sur les identifiants des occurrences 
	 */
	private Set<Integer> occurrenceIdsFilter;
	 ...
	 * @param limit
	 *            the maximum records to return for this query
	 */
	...
	public OccurrenceQuery(OccurrenceQuery other) {
		...
		orderingMap = other.orderingMap == null ? null
				: new ArrayList<OrderKey>(other.orderingMap);
	}
	...
	public Set<String> getFiltersFromProperty(String property, User loggedinUser) {
		return getFiltersFromProperty(property, loggedinUser,
				ResultFilter.PUBLIC);
	}

	public Set<String> getFiltersFromProperty(String property,
			User loggedinUser, ResultFilter resultFilter) {
		AppConstants constants = ApplicationView.getConstants();
		Set<String> filters = new HashSet<String>();
		boolean isLoggedIn = loggedinUser != null;
		if (property != null) {
			if (property.equalsIgnoreCase(constants.AllOccurrences())) {
				this.resultFilter = isLoggedIn ? ResultFilter.BOTH
						: ResultFilter.PUBLIC;
			...
			} else if (property.equalsIgnoreCase(constants
				...
				filters.add("reviewed = true");
					...
					.MyPositivelyReviewed())) {
				this.resultFilter = resultFilter == ResultFilter.BOTH ? null
						: resultFilter;
				// filters.add("public = " + !privateOnly);
				// filters.add("ownerEmail = " + loggedinUser.getEmail());
				filters.add("validated = true");
	...
	/**
	 * Notified all {@link DataRequestListener} that register in this query to
	 * get data from the {@link DataSwitch}.
	 * 
	 * @param pageNum
	 *            the request data page number.
	 */
